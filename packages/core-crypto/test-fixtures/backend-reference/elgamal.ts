/**
 * TEST FIXTURE — vendored verbatim, not app code. Do not import from
 * `src/index.ts` or any app code; never ships in the mobile bundle (excluded
 * by not being referenced from anything outside this package's tests).
 *
 * Provenance:
 *   Source repo:   https://github.com/sheikhhossainn/evoting-simulation
 *   Source path:   backend/src/crypto/elgamal.ts
 *   Source commit: 22fd389c877ca5d16fd1d923bb1a5f92c6cd5717 (branch: main)
 *   Fetched:       2026-09-25
 *
 * Why this file exists here: packages/core-crypto/src/elgamal.test.ts proves
 * "the ported client prover's output is accepted by the REAL backend
 * verifier" (port-equivalence — see that file's own header and
 * docs/ARCHITECTURE.md §4 in evoting-mobile). That test needs
 * generateKeypair() from this exact file and verifyBallotValidity() from the
 * sibling zkp.ts. Vendoring a frozen copy here (vs. importing across a
 * `../../../backend` path that doesn't exist in this repo) is what makes the
 * test runnable in evoting-mobile at all — see evoting-mobile's
 * docs/UPSTREAM_SYNC.md "Known issue" section for the alternative that was
 * considered (known-answer vectors) and why this option was chosen instead.
 *
 * Refresh process: this file goes stale the moment evoting-simulation's
 * backend/src/crypto/elgamal.ts changes. To refresh: re-run the
 * sparse-checkout process in evoting-mobile's docs/UPSTREAM_SYNC.md for
 * `backend/src/crypto/elgamal.ts`, diff against this file, and if it
 * differs, replace the body below (keeping this header, with an updated
 * commit hash and fetched date) in the same change that updates
 * docs/UPSTREAM_SYNC.md and re-runs this package's tests.
 *
 * Everything below this line is the unmodified source file.
 */
