 ...
 * the UNCHANGED backend verifier — for every candidate position, and under
 * two independently generated keypairs. That is the "the mobile prover can
 * never drift from the verifier" property the migration depends on (D4 /
 * risk R4).
 *
 * evoting-mobile note (deviates from the upstream evoting-simulation
 * monorepo, where this file lives): the monorepo imports the backend modules
 * by relative path (`../../../backend/src/crypto/{elgamal,zkp}`), which only
 * resolves inside that monorepo. This repo does not vendor the backend, so
 * those two files are instead vendored verbatim, test-fixture-only, under
 * `test-fixtures/backend-reference/` — see that directory's file headers for
 * exact provenance (source commit, fetch date) and the refresh process, and
 * see `docs/UPSTREAM_SYNC.md` in this repo for why vendoring was chosen over
 * a known-answer-vector alternative. Nothing else in this file changed.
 */
...
import { generateKeypair } from "../test-fixtures/backend-reference/elgamal";
import { verifyBallotValidity } from "../test-fixtures/backend-reference/zkp";
